...
class Parser
{
    ...
    {
        @trigger_error(
            esc_html('The method "throwParseError" is deprecated. Use "parseError" and throw the exception in the caller instead'),
            E_USER_DEPRECATED
        );

        throw $this->parseError(esc_html($msg));
    }
    ...
    public function parseError($msg = 'parse error')
    {
        ...
        if ($this->peek('(.*?)(\n|$)', $m, $this->count)) {
            ...
            $e = new ParserException(esc_html("$msg: failed at `$m[1]` $loc"));
        ...
        $e = new ParserException(esc_html("$msg: $loc"));
        $e->setSourcePosition([$this->sourceName, $line, $column]);
    ...
    public function parse($buffer)
    {
        ...
        if ($this->utf8 && !preg_match('//u', $buffer)) {
            ...
            throw new ParserException(esc_html($message));
        }
    ...
    public function parseSelector($buffer, &$out, $shouldValidate = true)
    {
        ...
        if ($shouldValidate && $this->count !== strlen($buffer)) {
            throw $this->parseError(esc_html("`" . substr($buffer, $this->count) . "` is not a valid Selector in `$buffer`"));
        }
        ...
        if (isset($this->buffer[$this->count]) && $this->buffer[$this->count] === '@') {
            ...
            if (
                ...
                if (
                    ...
                    throw $this->parseError(
                        esc_html("Unknown directive `{$plain}` not allowed in `" . $this->env->type . "` block")
                    );
                }
    ...
    protected function assertPlainCssValid($parsed, $startPos = null)
    {
        ...
        if (! $parsed) {
            ...
            throw $this->parseError(esc_html($message));
        }
