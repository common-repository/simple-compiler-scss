...
class SourceMapGenerator
{
    ...
    public function saveMap($content)
    {
        ...
        if (! is_dir($dir)) {
            ...
            throw new CompilerException(
                esc_html(sprintf('The directory "%s" does not exist. Cannot save the source map.', $dir))
            );
        }

        // FIXME: proper saving, with dir write check!
        if (file_put_contents($file, $content) === false) {
            throw new CompilerException(esc_html(sprintf('Cannot save the source map to "%s"', $file)));
        }
    ...
    public function generateJson($prefix = '')
    {
        ...
        $jsonSourceMap = wp_json_encode($sourceMap, JSON_UNESCAPED_SLASHES);

        if (json_last_error() !== JSON_ERROR_NONE) {
            throw new \RuntimeException(esc_html(json_last_error_msg()));
        }
    ...
    protected function getSourcesContent()
    {
        ...
        foreach ($this->sources as $sourceFile) {
            $source = wp_remote_get($sourceFile);
            $source = (is_array($source) && isset($source['body'])) ? $source['body'] : file_get_contents($sourceFile);
            $content[] = $source;
        }
