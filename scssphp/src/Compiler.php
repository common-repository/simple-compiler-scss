...
class Compiler
{
    ...
    {
        $source = wp_remote_get($path);
        $source = (is_array($source) && isset($source['body'])) ? $source['body'] : file_get_contents($path);
    ...
    public function compileString($source, $path = null)
    {
        ...
        try {
            ...
            throw new CompilerException(esc_html($this->addLocationToMessage($e->getMessage())), 0, esc_html($e));
        }
    ...
    protected function missingSelectors()
    {
        foreach ($this->extends as $extend) {
            ...
            throw $this->error(esc_html("\"$origin\" failed to @extend \"$target\". The selector \"$target\" was not found."));
        }
    }
    ...
    protected function isWith($block, $with, $without)
    {
        if (isset($block->type)) {
            ...
            if ($block->type === Type::T_DIRECTIVE) {
                ...
                } elseif (isset($block->selectors) && preg_match(',@(\w+),ims', wp_json_encode($block->selectors), $m)) {
    ...
    protected function evalSelectors($selectors)
    {
        ...
        if ($this->shouldEvaluate) {
            ...
            try {
                ...
                throw $this->error(esc_html($e->getMessage()));
            }
    ...
    /**
     * Append a root directive like @use or @charset as near as the possible from the source code
     * (keeping before comments, @use and @charset coming before in the source code)
     *
     * @param string                                 $line
     * @param \ScssPhp\ScssPhp\Formatter\OutputBlock $out
     * @param array                                  $allowed
...
See http://bit.ly/ExtendCompound for details.";
            ...
            case Type::T_INCLUDE:
                ...
                if (! $mixin) {
                    throw $this->error(esc_html("Undefined mixin $name"));
                }
                ...
                if (! empty($mixin->parentEnv)) {
                    ...
                    throw $this->error(esc_html("@mixin $name() without parentEnv"));
                }
            ...
            case Type::T_ERROR:
                ...
                throw $this->error(esc_html("File $fname on line $line ERROR: $value\n"));

            default:
                throw $this->error(esc_html("unknown child type: $child[0]"));
        }
        ...
        if (($f = $this->getBuiltinFunction($normalizedName)) && \is_callable($f)) {
            ...
            if ($prototype === null || strtolower($normalizedName) !== $normalizedName) {
                ...
                if ($actualLibName !== $libName || strtolower($normalizedName) !== $normalizedName) {
                    ...
                    @trigger_error(esc_html($warning), E_USER_DEPRECATED);
                    $fname = $this->getPrettyPath($this->sourceNames[$this->sourceIndex]);
                    $line  = $this->sourceLine;
            ...
            if (\get_class($this) !== __CLASS__ && !isset($this->warnedChildFunctions[$libName])) {
                ...
                if ($needsWarning) {
                    if (method_exists(__CLASS__, $libName)) {
                        @trigger_error(esc_html(sprintf('Overriding the "%s" core function by extending the Compiler is deprecated and will be unsupported in 2.0. Remove the "%s::%s" method.', $normalizedName, $declaringClass, $libName)), E_USER_DEPRECATED);
                    } else {
                        @trigger_error(esc_html(sprintf('Registering custom functions by extending the Compiler and using the lib* discovery mechanism is deprecated and will be removed in 2.0. Replace the "%s::%s" method with registering the "%s" function through "Compiler::registerFunction".', $declaringClass, $libName, $normalizedName)), E_USER_DEPRECATED);
                    }
                }
            }
    ...
    protected function opColorColor($op, $left, $right)
    {
        ...
        foreach ([1, 2, 3] as $i) {
            ...
            switch ($op) {
                ...
                default:
                    throw $this->error(esc_html("color: unknown op $op"));
            }
        }
    ...
    public function compileValue($value, $quote = true)
    {
        ...
        switch ($value[0]) {
            ...
            default:
                throw $this->error('unknown value type: ' . wp_json_encode($value));
        }
    }
    ...
    public function get($name, $shouldThrow = true, Environment $env = null, $unreduced = false)
    {
        ...
        if ($shouldThrow) {
            throw $this->error(esc_html("Undefined variable \$$name" . ($maxDepth <= 0 ? ' (infinite recursion)' : '')));
        }
    ...
    public function setOutputStyle($style)
    {
        switch ($style) {
            ...
            default:
                throw new \InvalidArgumentException(esc_html(sprintf('Invalid output style "%s".', $style)));
        }
    }
    ...
    public function setFormatter($formatterName)
    {
        if (!\in_array($formatterName, [Expanded::class, Compressed::class], true)) {
            @trigger_error(esc_html('Formatters other than Expanded and Compressed are deprecated.'), E_USER_DEPRECATED);
        }
        @trigger_error(esc_html('The method "setFormatter" is deprecated. Use "setOutputStyle" instead.'), E_USER_DEPRECATED);
    ...
    public function registerFunction($name, $callback, $argumentDeclaration = null)
    {
        if (self::isNativeFunction($name)) {
            @trigger_error(esc_html(sprintf('The "%s" function is a core sass function. Overriding it with a custom implementation through "%s" is deprecated and won\'t be supported in ScssPhp 2.0 anymore.', $name, __METHOD__)), E_USER_DEPRECATED);
        }

        if ($argumentDeclaration === null) {
            @trigger_error(esc_html('Omitting the argument declaration when registering custom function is deprecated and won\'t be supported in ScssPhp 2.0 anymore.'), E_USER_DEPRECATED);
        }

        if ($this->reflectCallable($callback)->getNumberOfRequiredParameters() > 1) {
            @trigger_error(esc_html('The second argument passed to the callback of custom functions is deprecated and won\'t be supported in ScssPhp 2.0 anymore. Register a callback accepting only 1 parameter instead.'), E_USER_DEPRECATED);
        }
    ...
    protected function importFile($path, OutputBlock $out)
    {
        ...
        if (isset($this->importCache[$realPath])) {
            ...
            $code = (is_array($code) && isset($code['body'])) ? $code['body'] : file_get_contents($path);
    ...
    public function findImport($url, $currentDir = null)
    {
            ...
            foreach ($this->importPaths as $dir) {
                ...
                if (\is_callable($dir)) {
                    ...
                    if (! \is_null($file)) {
                        ...
                        @trigger_error(esc_html(sprintf('Returning a file to import for CSS or external references in custom importer callables is deprecated and will not be supported anymore in ScssPhp 2.0. This behavior is not compliant with the Sass specification. Update your "%s" importer.', $callableDescription)), E_USER_DEPRECATED);
        ...
        if ($this->legacyCwdImportPath) {
            ...
            if (!\is_null($path)) {
                @trigger_error(esc_html('Resolving imports relatively to the current working directory is deprecated. If that\'s the intended behavior, the value of "getcwd()" should be added as an import path explicitly instead. If this was used for resolving relative imports of the input alongside "chdir" with the source directory, the path of the input file should be passed to "compileString()" instead.'), E_USER_DEPRECATED);

                return $path;
            }
        }

        throw $this->error(esc_html("`$url` file not found for @use"));
    }
    ...
    private function checkImportPathConflicts(array $paths)
    {
        ...
        throw $this->error(esc_html("It's not clear which file to import. Found:\n" . implode("\n", $formattedPrettyPaths)));
    }
    ...
    public function throwError($msg)
    {
        ...
        throw $this->error(esc_html(...func_get_args()));
    }
    ...
    protected function handleImportLoop($name)
    {
        for ($env = $this->env; $env; $env = $env->parent) {
            ...
            if (realpath($file) === $name) {
                throw $this->error(esc_html('An @use loop has been found: %s imports %s'), esc_html($file), esc_html(basename($file)));
            }
        }
    }
    ...
    protected function callScssFunction($func, $argValues)
    {
        ...
        if (! empty($func->parentEnv)) {
            ...
            throw $this->error(esc_html("@function $name() without parentEnv"));
        }
    ...
    protected function callNativeFunction($name, $function, $prototype, $args)
    {
        ...
        @trigger_error(esc_html(sprintf('Returning a PHP value from the "%s" custom function is deprecated. A sass value must be returned instead.', $name)), E_USER_DEPRECATED);
    ...
    private function verifyPrototype(array $prototype, $positional, array $names, $hasSplat)
    {
        ...
        foreach ($prototype['arguments'] as $i => $argument) {
            ...
            if ($i < $positional) {
                if (isset($names[$name])) {
                    throw new SassScriptException(esc_html(sprintf('Argument $%s was passed both by position and by name.', $originalName)));
                }
            } elseif (isset($names[$name])) {
                $nameUsed++;
            } elseif ($default === null) {
                throw new SassScriptException(esc_html(sprintf('Missing argument $%s', $originalName)));
            }
        }
        ...
        if ($positional > \count($prototype['arguments'])) {
            ...
            if (!$hasSplat) {
                throw new SassScriptException(esc_html($message));
            }
        ...
        foreach ($args as $arg) {
            if (!empty($arg[0])) {
                ...
                if (isset($keywordArgs[$name])) {
                    throw new SassScriptException(esc_html(sprintf('Duplicate named argument $%s.', $arg[0][1])));
                }
                    ...
                    if (isset($val[3]) && \is_array($val[3])) {
                        foreach ($val[3] as $name => $item) {
                            ...
                            if (isset($keywordArgs[$normalizedName])) {
                                throw new SassScriptException(esc_html(sprintf('Duplicate named argument $%s.', $name)));
                            }
                ...
                throw new SassScriptException(esc_html('Positional arguments must come before keyword arguments.'));
            } else {
        ...
        if (! is_numeric($value)) {
            ...
            if ($value instanceof Number) {
                if ($value->unitless()) {
                    ...
                    throw $this->error(esc_html('Expected %s to have no units or "%%".'), esc_html($value));
                }
    ...
    public function assertString($value, $varName = null)
    {
        ...
        if (! \in_array($value[0], [Type::T_STRING, Type::T_KEYWORD])) {
            $value = $this->compileValue($value);
            throw SassScriptException::forArgument(esc_html("$value is not a string."), esc_html($varName));
        }
    ...
    {
        @trigger_error(esc_html(sprintf('"%s" is deprecated since 1.7.0.', __METHOD__)), E_USER_DEPRECATED);
    ...
    public function assertMap($value, $varName = null)
    {
        ...
        if ($map === null) {
            ...
            throw SassScriptException::forArgument(esc_html("$value is not a map."), esc_html($varName));
        }
    ...
    {
        if ($value[0] !== Type::T_LIST) {
            throw $this->error(esc_html('expecting list, %s received'), esc_html($value[0]));
        }
        assert(\is_array($value));
    ...
    public function getArgumentListKeywords($value)
    {
        if ($value[0] !== Type::T_LIST || !isset($value[3]) || !\is_array($value[3])) {
            throw new \InvalidArgumentException(esc_html('The argument is not a sass argument list.'));
        }
    ...
    public function assertColor($value, $varName = null)
    {
        ...
        throw SassScriptException::forArgument(esc_html("$value is not a color."), esc_html($varName));
    }
    ...
    public function assertNumber($value, $varName = null)
    {
        if (!$value instanceof Number) {
            $value = $this->compileValue($value);
            throw SassScriptException::forArgument(esc_html("$value is not a number."), esc_html($varName));
        }
    ...
    public function assertInteger($value, $varName = null)
    {
        ...
        if (round($value - \intval($value), Number::PRECISION) > 0) {
            throw SassScriptException::forArgument(esc_html("$value is not an integer."), esc_html($varName));
        }
    ...
    protected function libCall($args)
    {
        ...
        if (! in_array($functionReference[0], [Type::T_FUNCTION_REFERENCE, Type::T_FUNCTION])) {
            throw $this->error(esc_html('Function reference expected, got ' . $functionReference[0]));
        }
    ...
    protected function alterColor(array $args, $operation, $fn)
    {
        ...
        if ($hasRgb && ($hasSL || $hasWB || $hue !== null)) {
            throw new SassScriptException(esc_html(sprintf('RGB parameters may not be passed along with %s parameters.', $hasWB ? 'HWB' : 'HSL')));
        }

        if ($hasWB && $hasSL) {
            throw new SassScriptException(esc_html('HSL parameters may not be passed along with HWB parameters.'));
        }
...
will be an error in future versions of Sass.\n         on line $line of $fname";
    ...
    protected function libKeywords($args)
    {
        ...
        if ($value[0] !== Type::T_LIST || !isset($value[3]) || !\is_array($value[3])) {
            ...
            throw SassScriptException::forArgument(esc_html($compiledValue . ' is not an argument list.', 'args'));
        }
    ...
    protected function libRandom($args)
    {
        if (isset($args[0]) && $args[0] !== static::$null) {
            ...
            if ($limit->hasUnits()) {
                ...
                $message = "random() will no longer ignore \$limit units ($limit) in a future release.
...
More info: https://sass-lang.com/d/random-with-units";
            ...
            if ($n < 1) {
                throw new SassScriptException(esc_html("\$limit: Must be greater than 0, was $n."));
            }

            return new Number(wp_rand(1, $n), '');
        }

        $max = mt_getrandmax();
        return new Number(wp_rand(0, $max - 1) / $max, '');
    }
    ...
    protected function libUniqueId()
    {
        ...
        if (! isset($id)) {
            $id = PHP_INT_SIZE === 4
                ? wp_rand(0, pow(36, 5)) . str_pad(wp_rand(0, pow(36, 5)) % 10000000, 7, '0', STR_PAD_LEFT)
                : wp_rand(0, pow(36, 8));
        }
    ...
    protected function getSelectorArg($arg, $varname = null, $allowParent = false)
    {
        ...
        if (! $this->checkSelectorArgType($arg)) {
            ...
            throw SassScriptException::forArgument(esc_html("$var_value is not a valid selector: it must be a string, a list of strings, or a list of lists of strings"), esc_html($varname));
        }
                ...
                foreach ($gluedSelector as $selector) {
                    foreach ($selector as $s) {
                        if (in_array(static::$selfSelector, $s)) {
                            throw SassScriptException::forArgument(esc_html("Parent selectors aren't allowed here."), esc_html($varname));
                        }
                    }
                }
            }
        ...
        throw SassScriptException::forArgument(esc_html("expected more input, invalid selector."), esc_html($varname));
    }
