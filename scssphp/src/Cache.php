...
class Cache
{
    ...
    public function getCache($operation, $what, $options = [], $lastModified = null)
    {
        ...
        if (
            ...
            if (
                ...
                $c = wp_remote_get($fileCache);
                $c = (is_array($c) && isset($c['body'])) ? $c['body'] : file_get_contents($fileCache);
                $c = unserialize($c);
    ...
    public static function checkCacheDir()
    {
        ...
        if (! is_dir(self::$cacheDir)) {
            throw new Exception('Cache directory doesn\'t exist: ' . esc_html(self::$cacheDir));
        }

        if (! is_writable(self::$cacheDir)) {
            throw new Exception('Cache directory isn\'t writable: ' . esc_html(self::$cacheDir));
        }
    }
